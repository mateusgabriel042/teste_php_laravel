<div id="messages">
    <a class="nav-link dropdown-toggle" href="#" id="messagesDropdown" role="button" data-toggle="dropdown"
        ...
        <!-- Counter - Messages -->
        @if (count(App\Helper\Helpers::messageList()) > 5)
            <span data-count="5" class="badge badge-danger badge-counter">5+</span>
        @else
            <span data-count="{{ count(App\Helper\Helpers::messageList()) }}"
                class="badge badge-danger badge-counter">{{ count(App\Helper\Helpers::messageList()) }}</span>
        @endif
    </a>
    <!-- Dropdown - Messages -->
    <div class="dropdown-list dropdown-menu dropdown-menu-right shadow animated--grow-in"
        ...
        <div id="message-items">
            @foreach (App\Helper\Helpers::messageList() as $message)
                <a class="dropdown-item d-flex align-items-center" href="{{ route('message.show', $message->id) }}">
                    <div class="dropdown-list-image mr-3">
                        @if ($message->photo)
                            <img class="rounded-circle" src="{{ $message->photo }}" alt="perfil">
                        @else
            ...
            <div>
                <div class="text-truncate">Eu sou um bom menino? A razão de eu perguntar é porque alguém me disse que as pessoas dizem isso para todos os cães, mesmo que eles não sejam bons...</div>
                <div class="small text-gray-500">Galinha o Cachorro · 2w</div>
            </div>
            </a> --}}
        </div>
        <a class="dropdown-item text-center small text-gray-500" href="{{ route('message.index') }}">Leia mais
            mensagens</a>
    </div>
</div>
